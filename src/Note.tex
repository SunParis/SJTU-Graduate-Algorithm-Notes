...
设 $P = \max_i v_i$，缩放因子 $K = \varepsilon P / n$（实际实现中取正整数尺度，若取整后为 0 则令 $K=1$），
令 $v_i' = \lfloor v_i / K \rfloor$。

对缩放价值 $v_i'$ 运行按价值 DP。因为
\[
  v_i'\le \frac{v_i}{K}\le \frac{P}{K}=\frac{n}{\varepsilon},
\]
缩放后的最大单个利润为 $O(n/\varepsilon)$，总利润上界为 $O(n^2/\varepsilon)$。
因此按价值 DP 的时间为
\[
  O\!\left(n\cdot \sum_i v_i'\right)=O(n^3/\varepsilon)
\]
或按课件用最大利润上界写成 $O(n^2\lfloor P/K\rfloor)=O(n^3/\varepsilon)$。
无论采用哪种等价记法，运行时间都关于 $n$ 和 $1/\varepsilon$ 多项式。

\textbf{误差分析：}设算法选出物品集合 $S$，最优集合为 $S^*$。
\[
  v_i-Kv_i' < K
  \quad\Longrightarrow\quad
  \sum_{i\in S^*}v_i - K\sum_{i\in S^*}v_i' < nK.
\]
DP 返回的集合 $S$ 在缩放利润下不差于 $S^*$，所以
\[
  \sum_{i\in S}v_i
...
\]
因此 $\OPT - A(I) \le Kn = \varepsilon P \le \varepsilon \OPT$，得到 $(1-\varepsilon)$-近似。

\subsection{上面例子上的缩放}

课件例子中 $P=3,n=5$。若取 $\varepsilon=1$，则 $K=\varepsilon P/n=0.6$，
向下取整利润为
\[
  (3,2,3,1,2)/0.6 \quad\leadsto\quad v'=(5,3,5,1,3).
\]
缩放后的利润上界从原来的 $nP=15$ 变成约 $n^2/\varepsilon=25$ 的量级。
这个小例子里数值本来就很小，缩放不一定显著减少表格；FPTAS 的意义在于当原始利润很大时，
例如利润有 $10^8$ 量级，删去低位信息后 DP 只依赖 $n$ 和 $1/\varepsilon$。

若为便于手算直接取 $K=2$，原利润 $(3,2,3,1,2)$ 变为
\[
  v'=(1,1,1,0,1).
\]
物品 D 的缩放利润为 $0$，DP 会更关注 A、B、C、E 的组合。
原最优 $\{A,B,E\}$ 利润 $7$、缩放利润 $3$；集合 $\{A,E\}$ 利润 $5$、缩放利润 $2$。
缩放会丢失利润细节，但每个物品损失小于 $K$，总损失小于 $nK$，这正是上面误差证明的来源。
...
课件给出一个重要硬度结论：若 $P\ne NP$，则任意 $\varepsilon>0$，
Bin Packing 不存在近似比 $3/2-\varepsilon$ 的多项式算法。

\proofidea{从 Partition 归约。给定非负数 $a_1,\dots,a_n$，
问是否能分成两组且每组和相等。把箱子容量设为总和的一半。
若 Partition 答案为 yes，则所有物品可装入 $2$ 个箱子。
若存在比 $3/2$ 略好的近似算法，那么在 yes 情况下，算法输出箱子数必须小于 $3$，
由于箱子数是整数，只能输出 $2$，于是可判定 Partition。
这会推出 $P=NP$。}

这个结论解释了为什么 Bin Packing 的精细近似通常采用\textbf{渐近}形式：
允许一个常数加法误差，而不是对所有小最优值实例都要求严格乘法比。

\subsection{APTAS：渐近近似方案}

\begin{definition}
对最小化问题，若算法族 $A_\varepsilon$ 满足
\[
  A_\varepsilon(I)\le (1+\varepsilon)\OPT(I)+c
\]
其中 $c$ 是与输入规模无关的常数，则称为 \textbf{APTAS}。
\end{definition}

Bin Packing 有 APTAS。课件证明的是：对任意 $0<\varepsilon\le 1/2$，
存在多项式时间算法使用至多
\[
...
\textbf{第一步：大物品且大小种类数固定时可精确求解。}
若所有物品大小至少为 $\varepsilon$，且只有 $K$ 种不同大小。
一个箱子最多装
\[
  M=\lfloor 1/\varepsilon\rfloor
\]
个物品。箱子类型由 $K$ 种物品各放几个决定，类型数至多
\[
  R=\binom{M+K}{M},
\]
当 $\varepsilon,K$ 固定时这是常数。总箱子数不超过 $n$，
枚举每种箱子类型使用多少个，组合数至多 $\binom{n+R}{R}$，关于 $n$ 是多项式。

\textbf{第二步：只有大物品但大小种类不固定时，分组取整。}
把物品按大小递增排序，分成
\[
  K=\lceil 1/\varepsilon^2\rceil
\]
组，每组至多 $Q=\lfloor n\varepsilon^2\rfloor$ 个。
将每组物品大小向上舍入为该组最大物品大小，得到实例 $J$，它只有 $K$ 种大小。
对 $J$ 精确求解，得到的装箱对原实例也可行。

为什么损失至多 $(1+\varepsilon)$？
令 $J'$ 为向下舍入到每组最小物品大小的实例，则 $\OPT(J')\le \OPT(I)$。
一个 $J'$ 的装箱可以转化为 $J$ 中除最大一组 $Q$ 个物品外的装箱，所以
\[
  \OPT(J)\le \OPT(J')+Q\le \OPT(I)+Q.
\]
因为每个大物品大小至少 $\varepsilon$，总大小至少 $n\varepsilon$，
故 $\OPT(I)\ge n\varepsilon$，从而 $Q\le n\varepsilon^2\le \varepsilon\OPT(I)$。
于是
\[
  \OPT(J)\le (1+\varepsilon)\OPT(I).
\]

\textbf{第三步：小物品用 First Fit 填缝。}
先丢开所有大小 $<\varepsilon$ 的小物品，只对大物品运行第二步算法。
然后把小物品按 First Fit 放入已开的箱子，必要时再开新箱子。
若没有新箱子，箱子数至多 $(1+\varepsilon)\OPT$。
若开了新箱子，最后一个箱子之外的每个箱子装载量都至少 $1-\varepsilon$，
否则当前小物品应能放进去。因此若总共用 $M$ 个箱子，总大小至少
\[
  (M-1)(1-\varepsilon).
\]
总大小是 $\OPT$ 的下界，所以
\[
  M\le \frac{\OPT}{1-\varepsilon}+1\le (1+2\varepsilon)\OPT+1
\]
其中最后一步用到 $0<\varepsilon\le 1/2$。
...
\textbf{例子：}全集 $U=\{a,b,c,d\}$，
$S_1=\{a,b\},S_2=\{b,c\},S_3=\{c,d\},S_4=\{a,d\}$，每个集合成本为 $1$。
每个元素出现频率 $f=2$。LP 解若为 $x_1=x_2=x_3=x_4=1/2$，
则每个元素被覆盖到总量 $1$。阈值 $1/f=1/2$ 会选出全部四个集合，成本 $4$。
最优整数解可取 $S_1,S_3$，成本 $2$。这个例子虽然不是紧的，但展示了阈值舍入的逻辑：
每个元素的分数覆盖总量至少为 $1$，而参与它的集合最多 $2$ 个，所以必有一个集合达到 $1/2$。
...
\qquad
\begin{array}{rll}
\max & \sum_{i=1}^m b_iy_i\\
\text{s.t.} & \sum_{i=1}^m a_{ij}y_i\le c_j, & j=1,\dots,n,\\
&y_i\ge0.
\end{array}
\]
精确互补松弛说：若 $x_j>0$，则对应对偶约束必须紧；
若 $y_i>0$，则对应原始约束必须紧。近似算法中通常达不到完全紧，
于是放宽为：
\[
  x_j>0 \Rightarrow \frac{c_j}{\alpha}\le \sum_i a_{ij}y_i\le c_j,
\]
\[
  y_i>0 \Rightarrow b_i\le \sum_j a_{ij}x_j\le \beta b_i.
\]
若原始和对偶可行且满足上述条件，则
\[
  \sum_j c_jx_j
  \le \alpha\sum_j\left(\sum_i a_{ij}y_i\right)x_j
  =\alpha\sum_i\left(\sum_j a_{ij}x_j\right)y_i
  \le \alpha\beta\sum_i b_iy_i
  \le \alpha\beta\OPT.
\]
因此近似比为 $\alpha\beta$。这就是课件中 primal-dual schema 的数学骨架。
...
证明关键是任取集合 $S$，设其有 $k$ 个元素，按被贪心覆盖的先后顺序记为
$e_1,\dots,e_k$。覆盖 $e_i$ 时，集合 $S$ 至少还含有 $k-i+1$ 个未覆盖元素，
所以若当轮选择 $S$，平均成本至多 $c(S)/(k-i+1)$。
贪心选择最划算集合，因此
\[
  \alpha_{e_i}\le \frac{c(S)}{k-i+1}.
\]
于是
\[
  \sum_{e\in S}y_e
  =\frac1{H_n}\sum_{i=1}^k\alpha_{e_i}
  \le \frac{c(S)}{H_n}\left(1+\frac12+\cdots+\frac1k\right)
  =\frac{H_k}{H_n}c(S)\le c(S).
\]
所以 $y$ 是对偶可行解。算法成本为
\[
  ALG=\sum_e\alpha_e=H_n\sum_e y_e\le H_n\OPT_f\le H_n\OPT.
\]

\textbf{Dual fitting 例子：}对 $U=\{e,f,g\}$，
$S_1=\{e,f\},S_2=\{f,g\},S_3=\{e,g\}$，单位成本。
贪心第一步任取 $S_1$，新覆盖 $e,f$，二者价格都是 $1/2$；
第二步可取 $S_2$ 覆盖 $g$，价格为 $1$。
算法总成本 $2=\alpha_e+\alpha_f+\alpha_g$。
直接价格对集合 $S_2=\{f,g\}$ 的总和为 $1/2+1=3/2>1$，对偶不可行。
除以 $H_3=1+1/2+1/3=11/6$ 后，
$S_2$ 上的对偶和变成 $(3/2)/(11/6)=9/11\le1$，所有集合约束都可行。
